...
11. **Delegate without losing coverage.** You may split a large change set or scope among read-only subagents, each with the same brief and a disjoint file list. Subagents never write or modify files. Re-read the full enclosing context of every finding they return before it enters the report; a subagent's finding is a lead, not evidence.
...
3. **Establish the target.** Change review: prefer explicit files or diff ranges from the operator. Otherwise compute the change set as the union of three lists: unstaged and staged changes versus HEAD, untracked files, and the merge-base diff against the default branch. All three are required — implementor and fixer agents that never commit leave their work unstaged and untracked, so commit-only diffs miss it, while working-tree diffs miss commits already made on the branch:
