...
12. **Never commit.** Do not stage files, create commits, push branches, or
    clean the worktree. Leave implementation changes dirty for the operator to
    review.
...
3. **Baseline the worktree.** Inspect `git status --short` and relevant diffs
   ...
   diff. Do not stage, stash, revert, or clean existing changes. If the
