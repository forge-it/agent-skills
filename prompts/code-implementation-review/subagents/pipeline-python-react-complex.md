...
Gate-runner brief:
    ...
    Discover the project's quality gates from its task runner and docs (justfile, Makefile, package.json scripts, CI config, CLAUDE.md). Run the standard set once on both sides — linters, type checkers, import contracts, and the test suites — scoped to the changed areas if a full run is impractical, and note any scoping you did. Write to <Y>-gate-results.md one line per gate: the exact command, PASS or FAIL, and for failures the first error with file:line. Fix nothing, change nothing. Run and record only. No preamble.
...
| 05 | /home/cristi/Projects/agent-skills/prompts/code-implementation-review/single-language/implementation-review-api-seam.md | <BACKEND> = Python |
