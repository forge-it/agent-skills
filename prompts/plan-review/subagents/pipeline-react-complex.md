...
Budget: at most 30 cheap subagents in total (2 + 7 + 8 + 3 + 1 + 8 + 1). The per-stage caps below enforce this.
...
|----|-------------|------------------------|
| 01 | /home/cristi/Projects/agent-skills/prompts/plan-review/single-language/plan-review-react-aggressive.md | — |
| 02 | /home/cristi/Projects/agent-skills/prompts/plan-review/single-language/plan-review-react.md | — |
| 03 | /home/cristi/Projects/agent-skills/prompts/plan-review/lenses/plan-review-security.md | — |
| 04 | /home/cristi/Projects/agent-skills/prompts/plan-review/lenses/plan-review-tests-and-migrations.md | — |
| 05 | /home/cristi/Projects/agent-skills/prompts/plan-review/generic/plan-review-loss-framing.md | — |
| 06 | /home/cristi/Projects/agent-skills/prompts/plan-review/generic/plan-review-day-off-bet.md | — |
| 07 | /home/cristi/Projects/agent-skills/prompts/plan-review/generic/plan-review-money-bet.md | — |

Wait for all 7 to finish before Stage 3.

Stage 3 — Work-item sweep (up to 8 subagents, parallel)

The panel reviews by concern; this stage reviews by slice, so nothing hides between concerns.

Read <Y>-plan-map.md. If it has more than 8 work items, merge the smallest related ones so you dispatch at most 8 sweepers. Launch all sweepers in a single message, each with this brief:
...
Dispatch one subagent with this brief:
    ...
    Read every review file <Y>-plan-review-*.md (including the wi- files) and every citation report <Y>-citation-check-*.md. Write <Y>-clusters.md. Merge findings that describe the same underlying issue into clusters CL-01, CL-02, … For each cluster record: the highest severity claimed, the issue in one sentence, the strongest citation, found-by k/M (list the review files; M is the total number of review files), citation health from the reports (VALID / BROKEN / UNCHECKABLE), and the affected work items (WI-NN). Do not add findings. Do not drop findings: anything that matches no cluster becomes its own cluster. Order by severity, then corroboration. Append a coverage matrix: work item × concern (separation-of-concerns, reactivity-types, ux-states, security, tests) marking where at least one finding or an explicit all-clear exists, and where neither does. No preamble.
