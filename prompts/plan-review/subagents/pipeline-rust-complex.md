...
Budget: at most 30 cheap subagents in total (2 + 8 + 7 + 3 + 1 + 8 + 1). The per-stage caps below enforce this.
...
| 02 | /home/cristi/Projects/agent-skills/prompts/plan-review/single-language/plan-review-rust.md | — |
...
| 05 | /home/cristi/Projects/agent-skills/prompts/plan-review/lenses/plan-review-operational-readiness.md | — |
| 06 | /home/cristi/Projects/agent-skills/prompts/plan-review/generic/plan-review-loss-framing.md | — |
| 07 | /home/cristi/Projects/agent-skills/prompts/plan-review/generic/plan-review-day-off-bet.md | — |
| 08 | /home/cristi/Projects/agent-skills/prompts/plan-review/generic/plan-review-money-bet.md | — |
...
Read <Y>-plan-map.md. If it has more than 7 work items, merge the smallest related ones so you dispatch at most 7 sweepers. Launch all sweepers in a single message, each with this brief:
...
Dispatch one subagent with this brief:
    ...
    Read every review file <Y>-plan-review-*.md (including the wi- files) and every citation report <Y>-citation-check-*.md. Write <Y>-clusters.md. Merge findings that describe the same underlying issue into clusters CL-01, CL-02, … For each cluster record: the highest severity claimed, the issue in one sentence, the strongest citation, found-by k/M (list the review files; M is the total number of review files), citation health from the reports (VALID / BROKEN / UNCHECKABLE), and the affected work items (WI-NN). Do not add findings. Do not drop findings: anything that matches no cluster becomes its own cluster. Order by severity, then corroboration. Append a coverage matrix: work item × concern (architecture, security, tests-migrations, operations) marking where at least one finding or an explicit all-clear exists, and where neither does. No preamble.
