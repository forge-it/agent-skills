...
3. **Choose the HTTP method** — see [HTTP Methods & Status Codes](#3-http-methods--status-codes) and the [PATCH vs. action sub-resource](#1-url-paths-critical) decision
...
8. **Decide idempotency and concurrency** — see [Idempotency](#9-idempotency) and [Concurrency Control](#10-concurrency-control)
9. **Verify against the checklist** — see [Pre-Commit Checklist](#pre-commit-checklist)
...
**How to decide: PATCH vs. action sub-resource?**

Ask: *"Does this operation represent a domain action or state transition with its own side effects, validation rules, or lifecycle — beyond simply changing a field value?"*

- **No** — the caller is updating one or more fields on the resource → use `PATCH /resource/<id>`. Example: changing a user's email address.
- **Yes** — the operation triggers domain logic that goes beyond field assignment (cancellation releases inventory, initiates refunds, sends notifications) → use an action sub-resource `POST /resource/<id>/cancel`.

Do not over-proliferate action endpoints. `POST /users/123/change-email` is wrong when `PATCH /users/123` with `{"email": "..."}` communicates the same intent. Do not under-use them either — `PATCH /orders/123` with `{"status": "cancelled"}` hides business semantics behind a field update when the operation has its own side effects and lifecycle.
...
An operation is idempotent when making the same request multiple times produces the same intended effect as making it once.
...
| `DELETE` | Yes                           | After deletion, repeating has no further effect |
| `PATCH`  | Yes (when applying the same delta) | Same field values applied again yield the same state |
| `POST`   | **No**                        | Repeating may create duplicates or re-trigger side effects |

**Rules:**

- `POST` action sub-resources that trigger side effects (payments, notifications, state transitions, inventory changes) **must** accept an `Idempotency-Key` request header.
- The server **must** store the key and, on a repeated request with the same key, return the original response without re-executing the operation.
- Keys are opaque strings generated by the client (typically UUIDs).
- The server **should** expire stored keys after a documented retention period (e.g., 24 hours).

```
# Client sends an action with an idempotency key
POST /api/payments/pay-123/capture
Idempotency-Key: 7f2c9a4b-...

# Network failure — client retries with the same key
POST /api/payments/pay-123/capture
Idempotency-Key: 7f2c9a4b-...
# Server recognizes the key and returns the original 200 response
```

Resource-creation endpoints (`POST /resource`) that are not themselves idempotent **should** also accept `Idempotency-Key` when duplicate creation would be harmful (e.g., duplicate orders, duplicate charges).

### 10. Concurrency Control

When multiple clients can modify the same resource concurrently, the API **must** use optimistic concurrency to prevent lost updates.

**Mechanism:**

1. The server includes an `ETag` header in every `GET` response, representing the current version of the resource.
2. Clients include `If-Match: "<etag>"` in `PATCH` and `DELETE` requests.
3. If the resource has changed since the client's `GET`, the server returns `409 Conflict`.

```
# Client reads the resource
GET /api/storage/volumes/vol-abc
→ 200 OK
→ ETag: "v10"
→ {"id": "vol-abc", "name": "data", "size_gb": 100}

# Client updates with the version it read
PATCH /api/storage/volumes/vol-abc
If-Match: "v10"
→ 200 OK
→ ETag: "v11"

# Another client tries to update the stale version
PATCH /api/storage/volumes/vol-abc
If-Match: "v10"
→ 409 Conflict
```
...
- Resources that support `PATCH` **should** return an `ETag` header on `GET` and `POST` (creation) responses.
- When `If-Match` is present, the server **must** reject the request with `409` if the ETag does not match.
- When `If-Match` is absent and the resource supports ETags, the server **may** accept the request (for backwards compatibility) or reject with `428 Precondition Required` (for strict enforcement).
- ETag values are opaque strings. Common implementations use a version counter, a content hash, or a last-modified timestamp.

### 11. Batch Operations

When clients need to apply the same operation to multiple resources in a single request, use a batch endpoint.

**URL pattern:** `POST /resource/batch-<action>`
...
# Batch cancel
POST /api/protection/jobs/batch-cancel
{"ids": ["job-10", "job-11"]}
```

**Rules:**

- Batch endpoints use `POST` regardless of the underlying operation.
- The request body contains an `ids` array (or a more specific typed array when the operation needs per-item parameters).
- The response reports **per-item results**, not all-or-nothing, unless the endpoint explicitly documents transactional semantics.
- Each item result includes the resource `id` and either a `status` or an `error`.

```json
{
  "results": [
    {"id": "user-1", "status": "deleted"},
    {"id": "user-2", "status": "deleted"},
    {"id": "user-3", "error": {"code": "RESOURCE_NOT_FOUND", "message": "User 'user-3' not found."}}
  ]
}
```

- The HTTP status code reflects the request as a whole: `200` when the batch was processed (even if individual items failed), `422` when the request itself is malformed.
- Batch endpoints **should** accept `Idempotency-Key` when the underlying operation has side effects.
- Document the maximum batch size (e.g., 100 items per request).
...
- [ ] Action sub-resources with side effects accept **`Idempotency-Key`**
- [ ] Resources supporting PATCH return **`ETag`** on GET; PATCH/DELETE send **`If-Match`**
- [ ] Batch endpoints return **per-item results** and document max batch size
...
| 14 | POST action without idempotency key | Retries cause duplicate side effects | Accept `Idempotency-Key` header; deduplicate server-side |
| 15 | PATCH without concurrency control | Concurrent updates silently overwrite each other | Use `ETag` / `If-Match`; return `409` on version mismatch |
| 16 | All-or-nothing batch with no per-item results | Clients can't tell which items failed | Return per-item `results` with individual status or error |
| 17 | PATCH with `{"status": "cancelled"}` for domain actions | Hides business semantics behind a field update | Use an action sub-resource: `POST /resource/<id>/cancel` |
